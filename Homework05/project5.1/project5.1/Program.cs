...
namespace project5._1
{
    ...
    static class OrderService
    {
        ...
        public static void DisplayOrder(OrderItems order)//显示该订单信息
        {
            try
            {
                if (order == null)
                ...
                Console.WriteLine("订单号: " + order.OrderNum);
                Console.WriteLine("客户姓名: " + order.ClientName);
                Console.WriteLine("--------------------------");
                Console.WriteLine("鲜花类型: " + order.Variety.ToString());
                Console.WriteLine("单价: " + order.Price);
                Console.WriteLine("数量: " + order.ProductsNum);
                Console.WriteLine("总价: " + order.Sum);
                Console.WriteLine("=====================================================");
                Console.WriteLine();
            }
            catch (DataException e)
        ...
        public static void FindOrderByProductVariety(Products variety)//查询包含该产品的订单，显示订单内容
        {
            bool exit = false;
            foreach (var od in orders)
            {
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (n == 1)
            {
                ...
                try
                {
                    OrderService.DisplayOrder(OrderService.FindOrderByOrderNum(ordernum01));
                }
                catch (DataException e)
                {
